...
const ProductList = React.createClass({
    ...
    render() {
        ...
        return (
            <div id="page-wrapper">
                {/*标题栏*/}
                <PageTitle pageTitle="Commodity management">
                    <div className="page-header-right">
                        <Link className="btn btn-primary" to="/product/save"><i className="fa fa-plus fa-fw"></i>Adding product</Link>
                    </div>
                </PageTitle>
                <div className="row">
                    {/*查询按钮*/}
                        ...
                        <div className="form-inline">
                            <div className="form-group">
                                <select className="form-control" onChange={this.onSearchTypeChange}>
                                    ...
                                    <option value="productName">Search by commondity name</option>
                                </select>
                            </div>
                            <div className="form-group">
                                <input type="text" className="form-control" placeholder="Key words" onChange={this.onKeywordChange}/>
                            </div>
                                    ...
                                    this.state.list.length ? this.state.list.map((product, index) => {
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{product.price}</td>
                                                <td>
                                                    <span>{product.status == 1 ? 'In selling' : 'Off selling'}</span>
                                                    <a className="btn btn-xs btn-warning opear"
                                                        data-status={product.status}
                                                        onClick={this.setProductStatus.bind(this, product.id, product.status)}>{product.status == 1 ? 'Not sale' : 'Sales'}</a>
                                                </td>
                                                <td>
                                                    <Link className="opear" to={ '/product/detail/' + product.id}>View</Link>
